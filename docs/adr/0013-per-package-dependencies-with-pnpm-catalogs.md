...
**Angular CLI placement:** `angular.json` lives at the root, but `@angular/cli` is declared in `packages/arcane-ui` and `apps/realm` — the projects that are built and tested via it. pnpm hoists binaries from all workspace packages to the root `node_modules/.bin`, so `ng` remains available at the workspace root as a first-class guarantee of pnpm's binary hoisting, not a side-effect of package hoisting.
...
- A future contributor seeing `@angular/cli` in `packages/arcane-ui/package.json` might wonder why it is there when `angular.json` is at the root. The answer is binary hoisting: the `ng` binary is available at root because pnpm hoists it there, not because the CLI is installed at root.
